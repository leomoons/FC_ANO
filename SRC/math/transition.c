...
Vector3f_t DCM_to_Euler_rad(float* dcm)
{
	...
	if(sy < 1e-6f)
	{
		euler.x = fast_atan2_lyh(-dcm[5], dcm[4]);
		euler.y = fast_atan2_lyh(-dcm[6], sy);
		euler.z = 0;
	}
	else
	{
		euler.x = fast_atan2_lyh(dcm[7], dcm[8]);
		euler.y = fast_atan2_lyh(-dcm[6], sy);
...
Vector3f_t Quater_to_Euler(float* quat)
{
	...
	euler.x = fast_atan2_lyh(sinr_cosp, cosr_cosp);
	...
	euler.z = fast_atan2_lyh(siny_cosp, cosy_cosp);
