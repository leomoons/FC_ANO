...
const u8  _cs_idlist[CSID_NUM]	 	= {0xF1,0xF2,0xF3,0xF4,0xF5,0xF6,0xFA};
//循环发送数据结构体
typedef struct
{
...
//	dt.txSet_u2[CSID_X20].fre_ms = 0;  //外部触发
	//position
	dt.txSet_u2[CSID_XF1].fre_ms = 0; //  
	//quatnion
	dt.txSet_u2[CSID_XF2].fre_ms = 10; //20ms
	//control force
	dt.txSet_u2[CSID_XF3].fre_ms = 0; //ms
	//control moment
	dt.txSet_u2[CSID_XF4].fre_ms = 10; //
	//estimation force 
	dt.txSet_u2[CSID_XF5].fre_ms = 0; //
	//estimation moment
	dt.txSet_u2[CSID_XF6].fre_ms = 10;//
	//velocity
	dt.txSet_u2[CSID_XFA].fre_ms = 0;//
	//pos
	//dt.txSet_u2[CSID_XF8].fre_ms = 50;//
//	//wind_vel
  //dt.txSet_u2[CSID_XF9].fre_ms = 100;//
	//电压
	//dt.txSet_u2[CSID_XFA].fre_ms = 200;//
	//传感器状态
...
//===========================================================
void ANO_DT_Send_Data7(u8 type,u8 *dataToSend , u8 length)
{
...
static void DTFrameAddData(u8 frame_num,u8 *_cnt)
{
	...
	switch(frame_num)
	{
		case 0xF1:		//位置
		{
			temp_data = (s16)(_state.pos_des.x);
		...
		case 0xF2:	//姿态四元数
		{
			temp_data = (s16)(_state.q_des[0]*10000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);		
			temp_data = (s16)(_state.q_des[1]*10000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);		
			temp_data = (s16)(_state.q_des[2]*10000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);		
			temp_data = (s16)(_state.q_des[3]*10000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);
			temp_data = (s16)(_state.q_fb[0]*10000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);
			temp_data = (s16)(_state.q_fb[1]*10000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);	
			temp_data = (s16)(_state.q_fb[2]*10000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);
			temp_data = (s16)(_state.q_fb[3]*10000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);
		}
		break;
		case 0xF3:		//机体坐标下的控制力
		{	
			temp_data = (s16)(_ctrl.F_b.x*1000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);		
			temp_data = (s16)(_ctrl.F_b.y*1000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);	
			temp_data = (s16)(_ctrl.F_b.z*1000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);
		}
		break;
		case 0xF4:	//控制力矩
		{
			temp_data = (s16)(_ctrl.M_b.x*10000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);		
			temp_data = (s16)(_ctrl.M_b.y*10000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);	
			temp_data = (s16)(_ctrl.M_b.z*10000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);
			CycleSendData[(*_cnt)++] = (char)(GetFlightMode());
		}
		break;
		case 0xF5:		//扰动观测力（世界坐标系）
		{
			temp_data = (s16)(_est.F_b.x*1000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);		
			temp_data = (s16)(_est.F_b.y*1000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);	
			temp_data = (s16)(_est.F_b.z*1000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);
		}
		break;
		case 0xF6:		//扰动观测力矩
		{
			temp_data = (s16)(_est.M_b.x*10000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);		
			temp_data = (s16)(_est.M_b.y*10000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);	
			temp_data = (s16)(_est.M_b.z*10000);
			CycleSendData[(*_cnt)++] = BYTE0(temp_data);
			CycleSendData[(*_cnt)++] = BYTE1(temp_data);
		}
		break;
...
static void DTFrameSend(u8 type, u8 id_addr, u8 addr)
{
	...
	ANO_DT_Send_Data7(type,CycleSendData, _cnt);
}
...
static void OtherSendDataCheck(void)
{
	for(u8 i=0; i<DT_ODNUM; i++)
	{
		...
		{
			ANO_DT_Send_Data7(OtherSendData[i].type,OtherSendData[i].data,OtherSendData[i].len);
...
void AnoDTRxOneByte(u8 type,u8 data)
{
	...
	switch (rx_anl[type].rxstate)
	{
		...
		case 1:
		{
			if((data == HW_TYPE || data == 0xFF || data == 0x05))
			{
...
static void AnoDTDataAnl(u8 type, u8 *data,u8 len)
{
  ...
  for(u8 i=0; i<len-2; i++)
	...
	else if(*(data+2)==0XE0)			//命令E0
	{
		...
		//需返回CHECK帧
		//SendCheck(type, 0xAF, *(data+2), check_sum1, check_sum2);
	}
